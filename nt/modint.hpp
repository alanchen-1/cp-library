...
struct mi{
    ...
    mi() {v = 0;}
    mi(const long long & _v){ v = (-MOD <= _v && _v < MOD) ? _v : _v % MOD; if (v < 0) v += MOD; }
	friend std::istream& operator>>(std::istream& in, mi& a) { long long x; std::cin >> x; a = mi(x); return in; }
	...
	mi& operator*=(const mi& m) { v = (long long)v * m.v % MOD; return *this; }
